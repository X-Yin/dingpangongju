...
const ALERT_THRESHOLD = 0.15; // 涨跌幅变化绝对值阈值（%）
...
const FloatingStockPosition = () => {
  ...
  const collapseTimerRef = useRef(null); // 延迟收缩计时器，避免边缘抖动导致鬼畜
  ...
  // 鼠标悬浮自动展开，移开自动收缩；移开延迟 200ms 再收缩，避免边缘抖动导致鬼畜
        ...
        if (
          ...
          if (Math.abs(diff) >= ALERT_THRESHOLD) {
            ...
            newAlertMap[item.code] = {
              ...
              text: `急速异动: ${sign}${Math.abs(diff).toFixed(2)}%`,
            };
          }
        }
      });
  ...
  useEffect(() => {
    ...
    fetchStockChanges();
    const timer = setInterval(fetchStockChanges, 2000);
    return () => clearInterval(timer);
  ...
  // 发生急速异动时自动展开（即使鼠标未悬浮），异动解除时自动收缩
  useEffect(() => {
    setExpanded(hasAnyAlert);
  }, [hasAnyAlert]);
