...
import { PlayCircleOutlined, PauseCircleOutlined, ReloadOutlined, AppstoreOutlined, BarChartOutlined, LineChartOutlined, StepBackwardOutlined, StepForwardOutlined, BackwardOutlined, ForwardOutlined } from '@ant-design/icons';
import { Button, Checkbox, Modal, Tabs, Empty } from 'antd';
...
const BlockMoneyChange = () => {
    ...
    const [playDirection, setPlayDirection] = useState(1); // 1 正放, -1 倒放
    const [isRealTime, setIsRealTime] = useState(true);
    const [selectedBlocks, setSelectedBlocks] = useState(DEFAULT_SELECTED_BLOCKS);
    const [viewMode, setViewMode] = useState('line'); // 'bubble' or 'bar' or 'line'
    const [modalBlock, setModalBlock] = useState(null); // 当前打开折线图弹窗的板块名
    const [activeTab, setActiveTab] = useState('intraday'); // 'intraday' 当日资金 | 'history' 历史资金
    ...
    }, [isPlaying, timeSeriesData.length, playDirection]);
    ...
    const handlePlay = (direction = 1) => {
        ...
        setCurrentIndex(prev => {
            if (direction > 0) {
                ...
                // 倒放：实时或已到开头时从末尾开始
                if (isRealTime || prev <= 0) {
                    return timeSeriesData.length - 1;
                }
                return prev;
            }
        });
        ...
        <Tabs
            ...
            <div className="floating-play-controls">
                ...
                <Button
                    ...
                    disabled={timeSeriesData.length === 0 || (isPlaying && playDirection > 0)}
                    className="floating-play-btn"
                    type={isPlaying && playDirection < 0 ? 'primary' : 'default'}
                >
                    {isPlaying && playDirection < 0 ? '暂停倒放' : '倒放'}
                </Button>
                ...
                <Button
                    type={isPlaying && playDirection > 0 ? 'primary' : 'default'}
                    icon={isPlaying && playDirection > 0 ? <PauseCircleOutlined /> : <PlayCircleOutlined />}
                    onClick={() => handlePlay(1)}
                    disabled={timeSeriesData.length === 0 || (isPlaying && playDirection < 0)}
                    className="floating-play-btn"
                >
                    {isPlaying && playDirection > 0 ? '暂停' : (isRealTime || currentIndex >= timeSeriesData.length - 1 ? '开始回放' : '继续')}
                </Button>
                {!isPlaying && !isRealTime && (
                    <Button
