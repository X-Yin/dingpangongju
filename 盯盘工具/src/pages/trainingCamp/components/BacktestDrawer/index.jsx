...
const SENTIMENT_BUY_RULES = [
  ...
  { key: 'market_gate', title: '大盘环境条件', desc: '创业板指 5 日线斜率 < 0 且 10 日线斜率 < 0，且银行板块（同花顺 881155）5 日线斜率 > 0 且较前一交易日更陡峭（斜率值增大；均按前一交易日收盘数据计算，斜率 = 当前 MA − 5 日前 MA）' },
...
const SENTIMENT_SELL_RULES = [
  ...
  { key: 'sentiment_stop_loss', title: '跌破成本线-5%', desc: '现价低于买入价 × 0.95（较买入价下跌 5%）即按触发分钟价格止损卖出' },
  { key: 'sentiment_break_ma5', title: '14:57 跌破 5 日线', desc: '14:57 后价格跌破 5 日线（按前一交易日收盘口径的 MA5）即卖出；分时未覆盖 14:57 时用当日最后一分钟判断；三者按分钟时序先到先卖' },
];
